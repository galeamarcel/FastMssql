...
def test_report_generator_preserves_not_run_and_redacts(
    ...
    assert (
        sum(line.startswith("| `") for line in matrix.splitlines()) == 377
    )
...
def test_report_generator_can_require_complete_evidence(
    ...
    assert "missing evidence for 377 case(s)" in completed.stderr
    assert matrix_output.is_file()
    assert report_output.is_file()
    assert "| NOT RUN | 377 |" in report_output.read_text(encoding="utf-8")
...
def test_approved_spec_contains_377_unique_case_ids() -> None:
    spec = ROOT / (
        "docs/superpowers/specs/"
        "2026-07-24-fastmssql-sql-auth-validation-design.md"
    )
